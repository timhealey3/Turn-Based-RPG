...
public:
    ...
    TEST_METHOD(Player_knight) {
        PlayerCharacter p1(new knight());
        Assert::AreEqual((int)knight::BASEHP, (int)p1.GetMaxHP());

        p1.GainEXP(100u);
        // level 2
        Assert::AreEqual(2, (int)p1.GetLevel());
        Assert::AreEqual(std::string("Power Attack"), p1.GetAbilityList().front()->GetName());
        Assert::AreEqual((int)(knight::BASEHP + (knight::BASEHP / 2.f)), (int)p1.GetMaxHP());

        p1.GainEXP(200u);
        // level 3
        Assert::AreEqual(3, (int)p1.GetLevel());
        Assert::AreEqual(std::string("Healing Surge"), p1.GetAbilityList()[1]->GetName());
        Assert::AreEqual((int)(knight::BASEHP + ((knight::BASEHP / 2.f) * 2)), (int)p1.GetMaxHP());
    }
